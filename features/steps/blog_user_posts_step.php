...
$steps->Given('/^ログインしていない$/', function($world) {
    return [
        new Given('"' . Router::url(['controller' => 'app_users', 'action' => 'logout'], true) . '" を表示している'),
    ];
});
$steps->When('/^"([^"]*)" の投稿を一覧表示する$/', function($world, $username) {
    $user = $world->getUser($username);
    return [
        new When('"' . Router::url(['controller' => 'posts', 'action' => 'index', 'user_account' => $user['username']], true) . '" を表示している'),
    ];
});
